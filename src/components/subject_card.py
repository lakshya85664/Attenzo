...
def subject_card(name, code, section, stats=None, footer_callback=None):
    ...
    if stats:
        ...
        for icon, label, value in stats:
            html+= f'<div style="background: #EB459E10; padding:5px 12px; border-radius:12px; font-size:0.9rem">{icon}  <span style="color: #EB459E;"><b></b>{value}</span> <span style="color: #EB459E;">{label}</span> </div>'
